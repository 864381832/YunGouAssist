...
<link href="css/ManagerESB/managerIndex.css" rel="stylesheet"
	type="text/css" />
<script type="text/javascript" src="js/AdminManager/managerAgent.js"></script>
<title>智能云购管理代理</title>
</head>
<body class="body">
	<div class="tableDiv">
		<div class="selectDiv">
			<form action="adminAction_managerAgent" target="_self" method="post"
				id="manageForm">
		...
		<h1>
			用户信息列表
			<lable> 代理数：${otherData.listSize}</lable>
		</h1>
		<table>
			<tr>
				<th>删除</th>
			...
			<s:iterator var="item" value="payRecordList">
				<tr>
					...
					<td><s:property value="#item.listSize" /></td>
					<td><s:property value="#item.RenewListNum" /></td>
					<td><s:property value="#item.RenewScale" />%</td>
					<td><s:property value="#item.RenewMoney" /></td>
					<td><s:property value="#item.RenewPayMoney" /></td>
					<td><s:property value="#item.RenewNotPayMoney" /></td>
					<td class="payIdTd">打款</td>
					<td class="remarksInfoIdTd"><s:property
